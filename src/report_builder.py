...
def build_telegraph_nodes(report: MacroReport) -> list:
    ...
    cpi_comment = ("持續下行，有助建立降息信心" if (cpi or 3) < 3.0
                   else "仍高於 Fed 2% 目標，降息時程受限")
    u_comment   = ("勞動市場韌性強，Fed 不急於降息" if (u or 4) < 4.0
                   else "開始降溫，有利降息空間")
    gdp_comment = ("超預期強勁" if (gdp or 2) > 2.5
                   else "成長放緩" if (gdp or 2) < 1.5 else "溫和擴張")
    ...
    nodes.append(_text(
        f"VOO（{report.voo_weight:.0f}%）：" +
        ("估值偏高，維持基礎倉位，等待回調" if report.voo_weight < 35 else "指標正常，維持定期扣款")
    ))
    nodes.append(_text(
        f"0050（{report.etf_0050_weight:.0f}%）：" +
        ("受惠全球風險偏好改善" if report.etf_0050_weight >= 30 else "相對美股具防禦優勢")
    ))
    nodes.append(_text(
        f"00679B（{report.etf_00679b_weight:.0f}%）：" +
        ("通膨下行 + 衰退機率上升，美債為當前最佳避險工具" if report.etf_00679b_weight > 40
         else "維持基礎防禦倉位")
    ))
    nodes.append(_br())
...
def build_report_line_messages(report: MacroReport, report_url: str) -> list:
    """LINE 短摘要 + Flex 卡片（含 Telegraph 連結）"""
    now_tst = datetime.now(TST)
    weekday = ["週一","週二","週三","週四","週五","週六","週日"][now_tst.weekday()]
    ...
    cpi_c = ("#1D9E75" if (report.cpi_latest or 3) < 2.5 else
             "#EF9F27" if (report.cpi_latest or 3) < 3.5 else "#E24B4A")
    rec_c = ("#E24B4A" if (report.ny_fed_recession_prob or 0) > 35 else
             "#EF9F27" if (report.ny_fed_recession_prob or 0) > 20 else "#1D9E75")
    ...
    if report_url:
        link_button = [{
            "type": "button",
            "action": {"type": "uri", "label": "查看完整報告", "uri": report_url},
